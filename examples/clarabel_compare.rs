...
fn main() {
    ...
    let mut row_ord: Vec<(usize, ConstraintType)> = (0..m)
        .map(|i| (i, prob.constraint_types[i]))
        .collect();
    ...
    if matches!(info.status, SolverStatus::Solved | SolverStatus::AlmostSolved) {
        // primal feas
        let x = &solution.x;
        ...
        println!("  pf_abs={:.3e} pf_rel={:.3e} (max_ax={:.3e} max_b={:.3e})",
            max_pf, max_pf / (1.0 + max_ax.max(max_b)), max_ax, max_b);
        println!("  |x|_inf={:.3e}", x.iter().fold(0.0_f64, |a, &v| a.max(v.abs())));
    }
}
