...
fn run(name: &str, timeout: f64) {
    ...
    eprintln!(
        "\n===== {} (n={} m={}) wall={:.1}s status={:?} iters={} =====",
        name, prob.num_vars, prob.num_constraints, wall, res.status, res.iterations
    );
    ...
    eprintln!("postsolve_lsq      = {:.2}s (refine_postsolve_dual_lsq)", us(tb.postsolve_lsq_us));
    eprintln!("postsolve_recovery = {:.2}s (refine_postsolve_recovery)", us(tb.postsolve_recovery_us));
    eprintln!("postsolve_refine   = {:.2}s (refine_post_processing stage1+2)", us(tb.postsolve_refine_us));
    eprintln!("postsolve_krylov   = {:.2}s (refine_krylov_and_projection)", us(tb.postsolve_krylov_ir_us));
