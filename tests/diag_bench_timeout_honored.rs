...
//! 格上げ対象外。LocallyOptimal のみ Optimal 化 (KKT 近傍正規 status)。
...
fn suboptimal_with_valid_solution_stays_suboptimal_qps_benchmark() {
    ...
    assert_eq!(
        ...
        "SuboptimalSolution は収束証明ではないため QPS bench で偽PASS化してはいけない"
    );
}

#[test]
fn suboptimal_with_valid_solution_stays_suboptimal_bench_qplib() {
    let r_in = make(SolveStatus::SuboptimalSolution, vec![1.0; 4], 2.0);
    let r_out = apply_bench_status_promotion(r_in, 4, BenchPromotionPolicy::BenchQplib);
    assert_eq!(
        r_out.status,
        SolveStatus::SuboptimalSolution,
        "SuboptimalSolution は QPLIB bench でも偽PASS化してはいけない"
    );
}
