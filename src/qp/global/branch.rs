...
/// max-violation: x*[j] が現 box midpoint から最も離れた変数 j* を選ぶ。
/// 距離は box 幅で正規化 (= |x[j] - mid| / width) → スケール非依存。
/// 返り値 None: 全変数が midpoint 近傍 or box が too narrow → 分枝不能 (= ε-converged)。
pub(crate) fn select_branching_variable(node: &BBNode, x: &[f64]) -> Option<usize> {
    ...
    for j in 0..n {
        ...
        match best {
            None => best = Some((j, dev)),
            Some((_, d)) if dev > d => best = Some((j, dev)),
            _ => {}
        }
    }
    best.map(|(j, _)| j)
...
mod tests {
    ...
    #[test]
    fn picks_var_with_largest_normalized_deviation() {
        let n = BBNode::root(vec![(0.0, 10.0), (-1.0, 1.0), (0.0, 100.0)], 0.0);
        // 正規化距離: var0=|9-5|/10=0.4, var1=|0.9-0|/2=0.45, var2=|50-50|/100=0
        let x = vec![9.0, 0.9, 50.0];
        let j = select_branching_variable(&n, &x).expect("should branch");
        assert_eq!(j, 1, "var1 has largest normalized deviation");
    }
