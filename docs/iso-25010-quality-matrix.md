...
| Performance efficiency | Timing sentinels for postsolve, simplex stalls, bench timeout handling, nextest slow-timeout limits, memory regression tests. | `cargo nextest run --release --features parallel -E 'test(memory_regression) | test(diag_bench_timeout_honored) | test(diag_dfl001_postsolve_speedup) | test(diag_lp_simplex_stall_sentinel)' --test-threads 3`; heavy surveillance in `.github/workflows/test-heavy.yml`. | Wall-clock sentinels are machine-sensitive; treat failures as investigation triggers, not benchmark publication. |
