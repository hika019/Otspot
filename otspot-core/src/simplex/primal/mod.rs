...
pub(crate) fn two_phase_simplex(
    ...
    if sf.num_artificial == 0 {
        ...
        //
        // Compressed Phase I optimisation: when not all rows need artificials
        // (e.g., an Eq LP with bounded variables also has UB Le-rows that start
        // feasible), run Phase I only on the artificial-needing rows. For a
        // 2500×3500 all-Eq bounded LP this reduces Phase I from 6000×9500 to
        // 2500×6000 — roughly a 9× per-pivot speedup. After Phase I the basis is
        // lifted back to the full m-row space and verified; any residual
        // infeasibility (e.g., a structural variable exceeded its upper bound
        // during the unconstrained Phase I) causes a fallback to the standard
        // full-system Phase I.
        if sf.num_artificial < m {
            if let Some(lifted) = run_phase1_compressed(
                sf, problem, options, &a, &b, &c, &row_scale, &col_scale, &mut total_iters,
            ) {
                return lifted;
            }
            // Compressed Phase I failed or produced UB violations — fall through
            // to the standard full-system Phase I below.
            total_iters = 0; // reset; full Phase I starts fresh
        }
...
/// Run Phase I on the compressed subproblem containing only the rows that need
/// artificial variables (`sf.needs_artificial[i]`). After Phase I, lift the
/// result to the full `sf.m`-row standard form and verify primal feasibility.
///
/// Returns `Some(SolverResult)` if:
/// - The compressed Phase I completes with `obj = 0` (all artificials removed),
/// - The lifted full basis is primal-feasible (no UB violations: `x_b ≥ −PIVOT_TOL`
///   for all non-artificial rows).
///
/// Returns `None` when Phase I times out, declares Infeasible, or produces
/// a lifted basis with UB violations. The caller falls back to the standard
/// full-system Phase I.
///
/// Column index space: artificials are at `[sf.n_total .. sf.n_total + m_c]` in
/// the compressed system — identical to the full system (same `n_total`, same
/// number of artificials `m_c = sf.num_artificial`). No index remapping needed.
#[allow(clippy::too_many_arguments)]
fn run_phase1_compressed(
    sf: &StandardForm,
    problem: &LpProblem,
    options: &SolverOptions,
    a: &CscMatrix,
    ...
    // Build index structures for the compressed rows.
    let art_rows: Vec<usize> = (0..m).filter(|&i| sf.needs_artificial[i]).collect();
    let m_c = art_rows.len(); // = sf.num_artificial
    debug_assert_eq!(m_c, sf.num_artificial);

    let mut row_to_c = vec![usize::MAX; m];
    for (ic, &r) in art_rows.iter().enumerate() {
        row_to_c[r] = ic;
    }

    // Build compressed augmented Phase I matrix using only the n_shifted structural
    // columns (those that appear in Eq/Ge rows). UB-slack columns [n_shifted..n_total)
    // never appear in artificial rows and can be excluded from Phase I entirely.
    //
    // Artificial column remapping: instead of [n_total..n_total+m_c), place them at
    // [n_shifted..n_shifted+m_c). This makes the Phase I column space contiguous:
    //   [0..n_shifted):         structural columns (same indices as full system)
    //   [n_shifted..n_ext_c):   artificial columns (dense, no gap)
    //   n_ext_c = n_shifted + m_c
    //
    // The pricing loop then covers n_ext_c = n_shifted + m_c columns instead of
    // n_total + m_c, eliminating the O(n_ub) wasted work on all-zero UB slack columns.
    let n_shifted = sf.n_shifted;
    let n_ext_c = n_shifted + m_c; // e.g. 3500 + 2500 = 6000

    let mut tc_rows: Vec<usize> = Vec::new();
    let mut tc_cols: Vec<usize> = Vec::new();
    let mut tc_vals: Vec<f64> = Vec::new();

    // Only include structural columns [0..n_shifted) that appear in artificial rows.
    // UB-slack columns [n_shifted..n_total) are skipped — zero entries in these rows.
    ...
    let b_c: Vec<f64> = art_rows.iter().map(|&r| b[r]).collect();
    let mut basis_c: Vec<usize> = art_rows.iter().map(|&r| sf.initial_basis[r]).collect();
    let mut x_b_c = b_c.clone();
    // Artificials at [n_shifted..n_shifted+m_c) in the compressed column space.
    for ic in 0..m_c {
        tc_rows.push(ic);
        tc_cols.push(n_shifted + ic);
        tc_vals.push(1.0);
        basis_c[ic] = n_shifted + ic;
    }

    let a_c = match CscMatrix::from_triplets(&tc_rows, &tc_cols, &tc_vals, m_c, n_ext_c) {
        Ok(m) => m,
        Err(_) => return None,
    };

    // Crash basis on compressed system. Artificials are now at [n_shifted..n_ext_c),
    // so pass n_total_c = n_shifted to tell try_apply_crash where artificials start.
    let crashed = if options.warm_start.is_none() && options.use_lp_crash_basis {
        try_apply_crash(
            &a_c,
            m_c,
            n_shifted,
            n_shifted, // n_total_c: structural ends at n_shifted in the compressed scheme
            &b_c,
            options.max_etas,
            options.deadline,
            &basis_c,
    ...
    if let Some((cb, cxb)) = crashed {
        basis_c = cb;
        x_b_c = cxb;
    } else {
        for i in 0..m_c {
            if let Ok((rows, vals)) = a_c.get_column(basis_c[i]) {
                for (k, &row) in rows.iter().enumerate() {
                    if row == i && vals[k].abs() > SLACK_DIAG_TOL {
                        x_b_c[i] /= vals[k];
                        break;
    ...
    // Charnes perturbation for compressed Phase I.
    // Artificials are at [n_shifted..n_ext_c) in the compressed scheme.
    for i in 0..m_c {
        if basis_c[i] >= n_shifted && x_b_c[i].abs() <= PIVOT_TOL {
            x_b_c[i] = PIVOT_TOL * (i as f64 + 1.0);
        }
    }

    let mut c_phase1_c = vec![0.0_f64; n_ext_c];
    c_phase1_c[n_shifted..].fill(1.0);

    let mut pricing1 = SteepestEdgePricing::new(n_ext_c);
    let phase1_outcome = revised_simplex_core(
        &a_c,
        &mut x_b_c,
        &c_phase1_c,
    ...
    // Phase I must reach Optimal with obj ≈ 0 for the compressed path to continue.
    match phase1_outcome {
        SimplexOutcome::Optimal(obj1, _) if obj1 > PIVOT_TOL => {
            return None;
        }
        SimplexOutcome::Optimal(_, _) => {
        }
    ...
    // Reconcile compressed basis to verify true feasibility.
    {
        let mut y_dummy = vec![0.0_f64; m_c];
        match reconcile_final_basis_state(
            &a_c,
            &b_c,
            &c_phase1_c,
        ...
        let rec_obj: f64 = (0..m_c)
            .map(|i| c_phase1_c[basis_c[i]] * x_b_c[i].max(0.0))
            .sum();
        if rec_obj > PIVOT_TOL {
            return None;
        }
    ...
    // Lift compressed Phase I basis → full m-row basis.
    // Column indices are identical (artificials live in [n_total..n_total+m_c]
    // in both the compressed and full systems).
    let mut basis_full = sf.initial_basis.clone();
    for (ic, &r) in art_rows.iter().enumerate() {
        basis_full[r] = basis_c[ic];
    }

    // Build the full augmented Phase I matrix for Phase II (same as the standard
    // full-system path). This is needed so Phase II can use the lifted basis.
    let mut tf_rows: Vec<usize> = Vec::new();
    let mut tf_cols: Vec<usize> = Vec::new();
    let mut tf_vals: Vec<f64> = Vec::new();
    for j in 0..a.ncols {
        if let Ok((rows, vals)) = a.get_column(j) {
            for (k, &row) in rows.iter().enumerate() {
                tf_rows.push(row);
    ...
    let mut art_col_full = n_total;
    for i in 0..m {
        if !sf.needs_artificial[i] {
            continue;
        }
        tf_rows.push(i);
        tf_cols.push(art_col_full);
        tf_vals.push(1.0);
        art_col_full += 1;
    }
    let n_ext = art_col_full;
    let a_ext = match CscMatrix::from_triplets(&tf_rows, &tf_cols, &tf_vals, m, n_ext) {
        Ok(m) => m,
        Err(_) => return None,
    };

    // Recompute x_b for the full system via LU(a_ext, basis_full).
    let mut x_b_full = {
        let bm = match LuBasis::new_timed(&a_ext, &basis_full, options.max_etas, options.deadline)
        {
            Ok(bm) => bm,
            Err(_) => return None,
        };
        ...
        let mut bm_mut = bm;
        bm_mut.ftran(&mut rhs);
        rhs.to_dense()
    };

    // Verify: all rows must be primal-feasible (x_b ≥ -PIVOT_TOL).
    // UB rows (non-artificial) might be infeasible if a structural variable
    // exceeded its upper bound during the unconstrained Phase I; in that case
    // we cannot safely start the primal Phase II and must fall back.
    if x_b_full.iter().any(|&v| v < -PIVOT_TOL) {
        return None;
    }

    // Clamp tiny negatives (numerical noise) to 0.
    for v in x_b_full.iter_mut() {
        if *v < 0.0 {
            *v = 0.0;
        }
    }

    // Pivot out any degenerate artificials remaining after compressed Phase I.
    pivot_out_degenerate_artificials(&a_ext, &mut basis_full, &x_b_full, sf, options);

    // Phase II (same as the standard full-system path).
    let mut c_phase2 = vec![0.0_f64; n_ext];
    c_phase2[..n_total].copy_from_slice(c);
    {
        let mut y_t = vec![0.0_f64; m];
        match reconcile_final_basis_state(
            &a_ext,
            b,
            &c_phase2,
    ...
    for i in 0..m {
        if x_b_full[i].abs() < PIVOT_TOL {
            x_b_full[i] = PIVOT_TOL * (i as f64 + 1.0);
        }
    }
    for v in x_b_full.iter_mut() {
        if *v < 0.0 {
    ...
    match phase2_outcome {
        SimplexOutcome::Optimal(obj2, mut y) => {
            ...
            let solution = extract_solution(sf, &basis_full, &x_b_full, col_scale);
            if !check_eq_feasibility(problem, &solution) {
                return None;
            }
            let (dual_solution, reduced_costs, slack) =
        ...
        SimplexOutcome::SingularBasis => None,
    }
}
