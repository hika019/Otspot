...
/// Run Big-M Phase I on the given (Ruiz-scaled) standard-form matrices and
/// return the primal-feasible basis if all artificials are driven out.
///
/// This is a stripped-down variant of `big_m_cold_start` that stops after
/// Phase I and does NOT run Phase II. The caller is responsible for Phase II.
///
/// Returns `Some((basis_structural, x_b, n_total, iters))` on success
/// (all artificials removed, basis in [0..n_total)):
/// - `basis_structural`: length `sf.m`, all entries < n_total (structural/slack).
/// - `x_b`: primal values for the basis (in augmented space; corresponds 1-to-1
///   with the original constraints since artificials are absent).
/// - `n_total = sf.n_total`.
/// - `iters`: total pivot count.
///
/// Returns `None` on Phase I failure (Timeout, Infeasible, singular basis,
/// or residual artificial in the final basis).
pub(crate) fn bigm_phase1_basis(
    sf: &crate::simplex::StandardForm,
    options: &SolverOptions,
    a: &CscMatrix,
    b: &[f64],
    c: &[f64],
) -> Option<(Vec<usize>, Vec<f64>, usize, usize)> {
    let m = sf.m;
    let n_total = sf.n_total;
    ...
    // Note: For bigm_phase1_basis, we intentionally skip the crash basis.
    // The LTSF crash covers ~99% of rows but produces an ill-conditioned
    // structural basis (dense LU, slow FTRAN/BTRAN) that is slower per
    // iteration than the identity (all-artificial) basis. The identity
    // basis has O(m) LU cost and allows faster convergence overall.
    // big_m_cold_start (the full solver) still uses crash for Phase II warm-start.
    let crash_state: Option<BigMPhase1State> = None;
    let BigMPhase1State {
        a_aug,
        mut basis_aug,
        c_aug_p1,
        mut x_b,
        artificial_col_of_row: _,
        ..
    } = match crash_state {
        Some(s) => {
            s
    ...
    // Charnes perturbation: rows with x_b ≈ 0 produce step=0 degenerate pivots
    // in Phase I, leading to Priority-1 lb-repair cycling. Nudge them to unique
    // positives so the ratio test produces step > 0 (mirrors the Phase II logic).
    for i in 0..m {
    ...
    // Performance optimisation: artificials (columns [n_total..n_aug)) cannot
    // re-enter the basis (n_enter = n_total). Passing n_price = n_total instead
    // of n_aug skips computing trow for the 2500 artificial columns in each
    // pricing pass, saving ~40% of the per-iteration pricing cost.
    // The `leaving_col < n_price` guards in the core handle the case where an
    // artificial leaves (leaving_col ≥ n_total) without out-of-bounds access.
    let n_price_p1 = n_total; // price only structural columns
    let mut leaving = ArtificialPriorityLeaving { n_total };
    let mut total_iters: usize = 0;
    let phase1_outcome = dual_simplex_core_advanced(
        &a_aug,
    ...
    match phase1_outcome {
        SimplexOutcome::Optimal(_, _) => {
            // Flush eta drift before handing basis to caller.
            if let Ok(mut bm) =
                LuBasis::new_timed(&a_aug, &basis_aug, options.max_etas, options.deadline)
            {
                let mut rhs = SparseVec::from_dense(b);
            ...
            // Verify no artificial remains.
            let any_art = (0..m).any(|i| basis_aug[i] >= n_total);
            if any_art {
                if farkas_infeasibility_certified(&a_aug, b, &basis_aug, m, n_total, options) {
                    return None; // LP is infeasible
                }
                return None; // couldn't eliminate all artificials
            }
            Some((basis_aug, x_b, n_total, total_iters))
        }
        _ => None,
    }
}
