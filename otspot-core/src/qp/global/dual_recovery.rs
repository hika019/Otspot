...
/// active 判定の許容幅。
///
/// incumbent は IPM が `user_eps` まで収束させた点なので、制約/bound からの距離が
/// `user_eps` スケール以内なら active とみなす。過剰に active と見なしても復元後の
/// complementarity 検証が採用を拒むため、この閾値は安全側にしか効かない。
fn active_tol(user_eps: f64, scale: f64) -> f64 {
    (user_eps * (1.0 + scale.abs())).max(FX_TOL)
}
...
pub(crate) fn recover_duals_at_fixed_x(
    ...
    if res.status != SolveStatus::Optimal || res.solution.len() != ncols {
        return None;
    }
